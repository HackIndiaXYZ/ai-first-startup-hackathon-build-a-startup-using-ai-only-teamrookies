...
export interface FounderPreferences {
  aiIntensity: number;       // 1 (Simple) to 10 (Advanced)
  technicalComplexity: number; // 1 (Simple) to 10 (Deep)
  riskAppetite: number;       // 1 (Safe) to 10 (Experimental)
  investmentPreference: number; // 1 (Low) to 10 (High)
  scalabilityPreference: number; // 1 (Local) to 10 (Global)
  mvpSpeed: number;           // 1 (Fast) to 10 (Slow/Careful)
  businessType: number;       // 1 (B2C) to 10 (B2B)
}

export interface FounderProfile {
  interests: string[]; // Weighted ordered list of interests
